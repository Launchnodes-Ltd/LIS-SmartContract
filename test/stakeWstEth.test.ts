...
export const stakeWStEthFlowTest = function () {
  /*-------------------------------------- Positive cases ---------------------------------------- */
  ...
  describe("Positive cases", () => {
    it("should return correct user balance", async () => {
      ...
      const stakeTx = await ngo.stakeWStEth(amountInETH, ngoPercentMultiplier);
      await stakeTx.wait();

      const stake2Tx = await ngo
        .connect(otherAccount)
        .stakeWStEth(amountInETH, ngoPercentMultiplier);
      await stake2Tx.wait();
    ...
    it("should be correct balance after 1 more day of rewards", async () => {
      ...
      const stakeTx = await ngo.stakeWStEth(
        amountInETH,
        NGO_PERCENT * PERCENT_MULTIPLIER
      );
      await stakeTx.wait();
      ...
      const stake2Tx = await ngo
        ...
        .stakeWStEth(
          anotherAmountOfEth,
          ANOTHER_NGO_PERCENT * PERCENT_MULTIPLIER
        );
      await stake2Tx.wait();
    ...
    it("should EMIT event Staked", async function () {
      ...
      await expect(ngo.stakeWStEth(amountInETH, ngoPercentMultiplier)).to.emit(
        ngo,
        "Staked"
      );
    });
    ...
    describe("Negative cases", () => {
      it("should REVERT if percent less then 1% or greater then 100%", async () => {
        ...
        await expect(
          ngo.stakeWStEth(amountInETH, minStakingPercent)
        ).to.revertedWithCustomError(ngo, "InvalidPercent");
        await expect(
          ngo.stakeWStEth(amountInETH, maxStakingPercent)
        ).to.revertedWithCustomError(ngo, "InvalidPercent");
      });
      ...
      it("should REVERT if NGO was finished", async () => {
        ...
        await expect(
          ngo.stakeWStEth(amountInETH, ngoPercentMultiplier)
